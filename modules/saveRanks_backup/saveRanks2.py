...
def savePricingHistory(data, vendor_id, update_query, update_values, pricing_data_for_history_list):
    try:
        ...
        if conn.is_connected():
            ...
            # Get the rank for this vendor_product_id from TempVendorPricing
            try:
                this.execute("""
                    SELECT rank 
                    FROM TempVendorPricing 
                    ...
                    LIMIT 1
                """, (vendor_product_id,))
                rank_result = this.fetchone()
                rank_value = rank_result[0] if rank_result else None
            except Exception as e:
                logger.debug(f"Error getting rank for vendor_product_id {vendor_product_id}: {e}")
                rank_value = None
            ...
            if result:
                ...
                # Update the rank in the history table
                if rank_value is not None:
                    try:
                        ...
                        """, (rank_value, result[0]))
                        conn.commit()
                        logger.debug(f"Updated rank to {rank_value} for vendor_product_id {vendor_product_id} in history table")
                    except Exception as e:
                        logger.debug(f"Error updating rank for vendor_product_id {vendor_product_id} in history table: {e}")
            else:
                if len(pricing_data_for_history_list) > 0:
                    # Update the rank in the pricing_data_for_history_list if available
                    if rank_value is not None:
                        # Find the index of 'rank' in the list (should be 17 based on the INSERT query)
                        # This is a bit fragile, but it's the best we can do without modifying the entire function
                        try:
                            # The rank is at index 17 in the pricing_data_for_history_list
                            pricing_data_for_history_list[17] = rank_value
                        except Exception as e:
                            logger.debug(f"Error updating rank in pricing_data_for_history_list: {e}")
                    ...
                    if this.rowcount == 1:
                        ...
                        # Update the rank in the newly inserted record
                        if rank_value is not None:
                            try:
                                this.execute("""
                                    UPDATE """+ pricing_history_table +"""
                                    SET rank = %s
                                    WHERE vendor_pricing_id = %s
                                """, (rank_value, this.lastrowid))
                                conn.commit()
                                logger.debug(f"Updated rank to {rank_value} for vendor_product_id {vendor_product_id} in newly inserted history record")
                            except Exception as e:
                                logger.debug(f"Error updating rank for vendor_product_id {vendor_product_id} in newly inserted history record: {e}")
    except mysql.connector.Error as e:
